...
async function VerificationEmail(email, password, code) {
    try {
        const request = await axios.post('https://app.leonardo.ai/api/auth/confirm-signup', {
            'email': email, 'password': password, 'confirmation_code': code
        }, {
            headers: {
        ...
        return {
            status: false, message: "Hesap doğrulanamadı", leonardaResponseMessage: error?.response?.data?.message,
        }
    }
